...
//            display.displayItem("<br> // Debug - spaceFree = " + this.spaceFree); //debug
        ...
        Open: function(filename){                                               //gets index of file in directory
            if(cDirectory=="Directory0"){index=C[0].filename.indexOf(filename);}
            else if(cDirectory =="Directory1"){index=C[1].filename.indexOf(filename);}
            else if(cDirectory =="C"){index=C[C.length-1].filename.indexOf(filename);}
            return index;
        },     
        Read: function(index){                                                  //using file index, get the file contents and return it
            if(cDirectory=="Directory0"){fileContent=C[0].content[index];}
            else if(cDirectory=="Directory1"){fileContent=C[1].content[index];}
            else if(cDirectory=="C"){fileContent=C[C.length-1].content[index];}
            return fileContent;
        },
        Create:function(NewFileName){                                           //adds an empty file to directory
            if(cDirectory=="Directory0"){C[0].filename.push(NewFileName);}
            else if(cDirectory=="Directory1"){C[1].filename.push(NewFileName);}
            else if(cDirectory=="C"){C[C.length-1].filename.push(NewFileName);}
        },
